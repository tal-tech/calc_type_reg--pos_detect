...
int main(){
    ...
    for (int i = 0; i < results.size(); ++i) {
        Json::Value position = results[i]["position"];
        std::cout << "Position for result " << i+1 << " is: ";
        std::cout << "{x: " << position["x"].asInt();
        std::cout << ", y: " << position["y"].asInt();
        std::cout << ", width: " << position["width"].asInt();
        std::cout << ", height: " <<
            position["height"].asInt() 
            <<" }" 
        <<std::endl;
    }
    ReleaseAiModel();
